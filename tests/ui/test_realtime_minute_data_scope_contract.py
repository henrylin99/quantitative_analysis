...
def test_realtime_child_pages_do_not_expose_1min_as_supported_period():
    ...
    # Each page should have a minute-data scope disclaimer
    assert "已入库的 Baostock 分钟数据进行展示" in monitor
    assert "已入库的分钟数据" in indicators
    assert "已入库的 Baostock 分钟数据生成与查看信号" in signals

    # 1min should not be exposed
