...
2. Replace all inline `<style>` with Obsidian tokens — remove the old dark mode block, old mobile styles that conflict, and the `stock-info` gradient
3. Update navbar HTML: replace Font Awesome chart-line icon with the Indigo gradient "Q" brand block, update brand text to "QuantAnalysis"
4. Update real-time indicator to green pill with "Live" text
5. Update refresh button to ghost style
6. Update ECharts theme registration to use Obsidian colors
7. Keep all responsive media queries but adjust colors/values for dark theme
...
| File | Change Type |
|------|------------|
| `app/templates/base.html` | Navbar redesign, font import, ECharts theme, inline styles |
| `app/static/css/financial-theme.css` | Full rewrite with Obsidian tokens |
...
| `app/templates/analysis.html` | Remove emoji, remove light-theme inline styles |
...
| `app/templates/ml_factor/text2sql/index.html` | Remove emoji, remove light-theme inline styles |
...
| `app/templates/realtime_analysis/risk_management.html` | Remove emoji, remove light-theme inline styles |
...
| `app/templates/data_management/index.html` | Remove emoji, remove light-theme inline styles |
...
- **Global CSS rewrite**: Touching `financial-theme.css` affects all pages. Mitigate by thorough visual testing across all page types.
- **Template emoji removal**: Some templates use emoji in button text or JS-generated HTML. Only remove emoji from heading text, not from functional JS strings.
