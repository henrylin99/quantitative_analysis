"""试用功能页数据计算服务 — 供 Flask 页面路由与 /api/trial JSON 接口共用。

计算口径与 main/views.py 中原实现保持一致（逻辑原样迁移），数据源为 data/data.parquet 大宽表最新交易日截面。
"""
...
def _wide_table_path():
    return Path(__file__).resolve().parents[2] / 'data' / 'data.parquet'


def _read_latest_day(required_cols):
    """读取大宽表并截取最新交易日，返回 (day_df, latest_trade_date)；缺列/空数据抛 ValueError。"""
    data_path = _wide_table_path()
    ...
    return day_df, latest_trade_date


def market_brief_payload():
    """每日市场简报：全市场概览 / TOP 榜 / 行业排名 / 特殊形态统计 + 简报文本。"""
    day_df, latest_trade_date = _read_latest_day([
    ...
    day_df = day_df[day_df['industry'].notna()].copy()
    if day_df.empty:
        raise ValueError(f'交易日 {latest_trade_date} 没有可用市场数据')
...
def financial_health_payload():
    """财务健康度评分卡：5 条规则 0-5 分，分布 + TOP50 排名。"""
    day_df, latest_trade_date = _read_latest_day([
    ...
    day_df = day_df.dropna(subset=['ts_code', 'name'])
    day_df = day_df[day_df['industry'].notna()].copy()
    if day_df.empty:
...
def stock_radar_payload(ts_codes):
    """个股对比雷达：2-4 只股票在估值/成长/技术/资金四维的标准化得分。"""
...
def stock_panorama_payload(ts_code):
    """个股全景：市场快照 + 四维分位雷达 + 财务/技术/资金/状态面板 + 形态标签。"""
    required_cols = [
    ...
    day_df, latest_trade_date = _read_latest_day(required_cols)
    ...
    def market_percentile(series, value, reverse=False):
...
def moneyflow_payload():
    """主力资金统计：行业汇总 + 净流入 TOP20 / 净流出 BOTTOM20。"""
    day_df, latest_trade_date = _read_latest_day([
